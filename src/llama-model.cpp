...
struct llama_model::impl {
    ...
    // RPC devices in registration order (TensorRelay model-part stubs)
...
ggml_tensor * llama_model_base::create_tensor(llama_model_loader & ml, const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) {
    ...
    if (weight != nullptr && weight->idx < ml.file_is_stub.size() && ml.file_is_stub[weight->idx]) {
        ...
        if (rpc_idx >= pimpl->rpc_devices.size()) {
            throw std::runtime_error(format(
                "tensor '%s' belongs to model-part stub shard %u but only %zu RPC device(s) are available",
                tn.str().c_str(), unsigned(weight->idx), pimpl->rpc_devices.size()));
        }
        auto it = pimpl->gpu_buft_list.find(pimpl->rpc_devices[rpc_idx]);
        if (it == pimpl->gpu_buft_list.end()) {
            throw std::runtime_error(format(
                "tensor '%s' belongs to model-part stub shard %u but RPC device %s has no buffer list",
                tn.str().c_str(), unsigned(weight->idx), ggml_backend_dev_name(pimpl->rpc_devices[rpc_idx])));
        }
